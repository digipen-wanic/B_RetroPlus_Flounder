...
namespace Levels
{
	...
	Level1::Level1() : Level("Level1"),
		...
		soundManager(nullptr), ambience(nullptr), energizerPositions(), dotPositions(),
	...
	void Level1::Load()
	{
		...
		soundManager->AddMusic("GhostAmbient.wav");
	...
	void Level1::Initialize()
	{
		...
		ambience = soundManager->PlaySound("GhostAmbient.wav");
	...
	void Level1::Update(float dt)
	{
		...
		if (readyTimer <= 0.0f)
		{
			...
			{
				// Play ambience once text disappears.
				if (!startedAmbience)
				{
		...
		if (!gameOver)
		{
			...
			if (pacMan->GetComponent<Behaviors::PlayerAnimation>()->IsDying())
			{
				ambience->setPaused(true);
			}
		}
